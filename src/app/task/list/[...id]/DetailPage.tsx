...
const DetailPage = ({ params, userData }: any) => {
  ...
  const handleStatusChange = (value: any) => {
    setStatus(value);
    setIsStatusChanged(value !== status);
  };
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if(error?.response?.data?.status === "notValidUser"){
      	setErrorAddingComment("You are not valid user to add a comment")
      }else{
      	setErrorAddingComment("Failed to add comment");
      }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if(error?.response?.data?.status === "notValidUser"){
      	setStatusError("You are not valid user to update status")
      }else{
      	setStatusError("Failed to update ");
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-[#0d1117] mt-4 sm:mt-32 p-4 sm:p-8">
      {loading ? (
        ...
        <div className="max-w-4xl w-full mx-auto p-6 bg-[#161b22] text-white shadow-lg rounded-lg mt-8 overflow-hidden">
          {fetchingError === "" ? (
            <div className="bg-[#21262d] shadow-inner rounded-lg p-4 sm:p-6 mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                ...
                <div className="pt-2 sm:pt-0">
                  <Button
                    ...
                    size="sm"
                    onClick={handleUpdate}
                    disabled={!isStatusChanged || isDeclined}
                    className="bg-blue-600 hover:bg-blue-700 text-white"
                  >
                    Update Status
                  </Button>
              ...
              <div className="mb-4 text-sm py-4 sm:py-5">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center">
                <div className="flex items-center text-end justify-end w-full sm:w-fit mt-5 space-x-2 text-green-700 mb-2 text-sm">
                  <div>Task created by:</div>
                  <div>
                    {data?.email === userData?.email ? "You" : data.email} on{" "}
                    {convertToDDMMYY(data.$createdAt)}
